...
export const GridDistortion: React.FC<GridDistortionProps> = ({ 
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (!renderer || !scene || !camera || !dataTextureRef.current) return;
    ...
    return () => {
      ...
      if (planeRef.current) {
          ...
          if (planeRef.current.material) (planeRef.current.material as THREE.Material).dispose();
      }
      if (dataTextureRef.current) dataTextureRef.current.dispose();
      if (uniformsRef.current && uniformsRef.current.uTexture.value) uniformsRef.current.uTexture.value.dispose();
